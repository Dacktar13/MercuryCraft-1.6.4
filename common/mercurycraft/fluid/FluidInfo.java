package mercurycraft.fluid;

public class FluidInfo {

	public static final String TEXTURE_LOCATION = "fluidMercury";

	public static int MERCURY_FLUID_ID;
	public static final String MERCURY_FLUID_KEY = "fluidMercury";
	public static final int MERCURY_FLUID_DEFAULT = 2520;
	public static final String MERCURY_FLUID_UNLOCALIZED_NAME = "fluidMercury";
	public static final String MERCURY_FLUID_NAME = "Mercury Fluid";
	public static final String MERCURY_FLUID_ICON = "fluidMercury";
	
	public static int BLOCK_MERCURY_FLUID_ID;
	public static final String BLOCK_MERCURY_FLUID_KEY = "blockfluidMercury";
	public static final int BLOCK_MERCURY_FLUID_DEFAULT = 2521;
	public static final String BLOCK_MERCURY_FLUID_UNLOCALIZED_NAME = "blockfluidMercury";
	public static final String BLOCK_MERCURY_FLUID_NAME = "Block Mercury Fluid";
	public static final String BLOCK_MERCURY_FLUID_ICON = "blockfluidMercury";
		
	public static int BIOME_MERCURY_DESERT_ID;
	public static final String BIOME_MERCURY_DESERT_KEY = "mercuryDesert";
	public static final int BIOME_MERCURY_DESERT_DEFAULT = 2522;
	public static final String BIOME_MERCURY_DESERT_UNLOCALIZED_NAME = "mercuryDesert";
	public static final String BIOME_MERCURY_DESERT_NAME = "Desert Biome with Mercury";
	public static final String BIOME_MERCURY_DESERT_ICON = "mercuryDesert";
	
	public static int BIOME_MERCURY_OCEAN_ID;
	public static final String BIOME_MERCURY_OCEAN_KEY = "mercuryOcean";
	public static final int BIOME_MERCURY_OCEAN_DEFAULT = 2523;
	public static final String BIOME_MERCURY_OCEAN_UNLOCALIZED_NAME = "mercuryOcean";
	public static final String BIOME_MERCURY_OCEAN_NAME = "Ocean Biome with Mercury";
	public static final String BIOME_MERCURY_OCEAN_ICON = "mercuryOcean";

}
